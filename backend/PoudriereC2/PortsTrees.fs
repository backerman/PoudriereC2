...
type PortsTreesApi (cfg: PortsRepository) =
    ...
    member _.NewPortsTree
        ([<HttpTrigger(AuthorizationLevel.Anonymous, "post", Route="portstrees")>]
    ...
    member _.ModifyPortsTree
        ([<HttpTrigger(AuthorizationLevel.Anonymous, "put", Route="portstrees/{treeId}")>]
         req: HttpRequestData, execContext: FunctionContext, treeId: Guid) =
        let log = execContext.GetLogger()
        async {
            let! maybeConfig = tryDeserialize<PortsTree> req log
            let response = req.CreateResponse(HttpStatusCode.OK)
    ...
    member _.DeletePortsTree
        ([<HttpTrigger(AuthorizationLevel.Anonymous, "delete", Route="portstrees/{treeId}")>]
         req: HttpRequestData, execContext: FunctionContext, treeId: Guid) =
        let log = execContext.GetLogger()
        async {
